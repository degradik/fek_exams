...
[...new Set(tests.map(t=>t.subject))].forEach(subject=>{
  ...
  tests.filter(t=>t.subject===subject).forEach((t,i)=>{
    const index=tests.indexOf(t),b=document.createElement('button');
    b.className='variant-link '+colors[i%3];b.dataset.start=index;
    b.innerHTML='<span>Вариант '+(i+1)+'</span><strong></strong><small></small>';
    b.querySelector('strong').textContent=t.title.split('·')[0].trim();
    b.querySelector('small').textContent=t.questions.length+' заданий · 100 баллов';
    b.addEventListener('click',()=>start(index));
    section.querySelector('.variant-grid').appendChild(b);
  });
...
function start(index){
  ...
  document.getElementById('correct').textContent='0';document.getElementById('points').textContent='0';document.getElementById('answered').textContent='0';
  document.getElementById('progress').style.width='0';document.getElementById('result').classList.remove('show');
  test.questions.forEach((q,i)=>render(q,i,test,totalPoints));
  home.classList.add('hidden');view.classList.remove('hidden');scrollTo(0,0);
  clearInterval(timerId);started=Date.now();tick();timerId=setInterval(tick,1000);
}
function render(q,i,test,totalPoints){
  const c=document.createElement('article');c.className='question';c.id='q-'+(i+1);
  c.innerHTML='<div class="q-head"><span class="q-number">'+String(i+1).padStart(2,'0')+'</span><span class="topic"></span></div><p class="prompt"></p><div class="options"></div><div class="solution-slot"></div><div class="feedback" aria-live="polite"></div>';
  c.querySelector('.topic').textContent=q.topic+' · '+(q.points||4)+' б.';
  c.querySelector('.prompt').textContent=q.prompt;
  const o=c.querySelector('.options');
  q.options.forEach((x,j)=>{
    const l=document.createElement('label');l.className='option';
    l.innerHTML='<input type="radio" name="q'+i+'" value="'+j+'"><span></span>';
    l.querySelector('span').textContent=x;
  ...
  if(q.solution){
    ...
    details.querySelector('.solution-text').textContent=q.solution;
    details.addEventListener('toggle',()=>{details.querySelector('summary').textContent=details.open?'Скрыть решение':'Показать решение'});
    c.querySelector('.solution-slot').appendChild(details);
  }
  root.appendChild(c);
}
function answer(i,selected,c,test,totalPoints){
  if(state.locked.has(i))return;state.locked.add(i);state.answered++;
  const q=test.questions[i],ok=selected===q.answer,labels=[...c.querySelectorAll('.option')];
  labels.forEach((l,j)=>{l.classList.add('locked');l.querySelector('input').disabled=true;if(j===q.answer)l.classList.add('correct');if(j===selected&&j!==q.answer)l.classList.add('wrong')});
